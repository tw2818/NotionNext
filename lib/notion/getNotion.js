...
export async function getNotion(pageId) {
  ...
  return {
    ...
    lastEditedTime: formatDate(new Date(postInfo?.last_edited_time).toLocaleTimeString().format('YYYY-MM-DD hh:mm:ss'), BLOG.LANG),
    fullWidth: false,
    page_cover: getPageCover(postInfo),
    date: { start_date: formatDate(new Date(postInfo?.last_edited_time).toLocaleTimeString().format('YYYY-MM-DD hh:mm:ss'), BLOG.LANG) },
    blockMap
  }
}
